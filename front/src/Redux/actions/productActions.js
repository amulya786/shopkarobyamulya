...
// keyword ki value default iss liye taki home par getproduct load ho sake
export const getProduct = ( priceRange = [0, 0],keyword = "", category="", page = 1) => async (dispatch) => {
    try {
        dispatch({
...
//review Action by admin
export const getAllReview = (id) => async (dispatch) => {
    try {
        dispatch({
            type: ALL_REVIEW_REQUEST
        });
        const { data } = await axios.put(`/api/v1/reviews?id=${id}`);
...
//Delete review Action by admin
export const deleteReview = (reviewsId,productId) => async (dispatch) => {
    try {
        dispatch({
            type: DELETE_REVIEW_REQUEST
        });
        const { data } = await axios.put(`/api/v1/reviews?id=${reviewsId}&productId=${productId}`);
